    ...
    def _propagate_from(
        ...
        with open(out_name, "wb") as fout, open(err_name, "wb") as ferr:
            ...
            if exe.poll() is None or exe.returncode == 0:
                ...
                frc_reader = ReadAndProcessOnTheFly(
                    out_files["frc"], xyz_reader
                )
                ...
                while exe.poll() is None or iterations_after_stop <= 1:
                    ...
                    frc_traj += frc_reader.read_and_process_content()
                    # loop over the frames that are ready
                    for frame in range(min(len(pos_traj), len(vel_traj), len(frc_traj))):
                        pos = pos_traj.pop(0)
                        vel = vel_traj.pop(0)
                        frc = frc_traj.pop(0)
                        write_xyz_trajectory_frc(traj_file, pos, vel, frc, atoms, box)
